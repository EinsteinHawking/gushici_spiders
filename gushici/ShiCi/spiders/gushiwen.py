...
class TextSpider(scrapy.Spider):
	...
	def parse(self, response):
		...
		for information in informations:
			...
			item['author'] = information.xpath('div[@class="cont"]/p[@class="source"]/a[last()]/text()').extract_first()
			item['dynasty'] = information.xpath('div[@class="cont"]/p[@class="source"]/a[1]/text()').extract_first()
			item['content'] = information.xpath('./div[@class="cont"]/div[@class="contson"] | p//text()').extract_first()
			item['tags'] = information.xpath('./div[@class="tag"]/a//text()').extract()


			item['author'] = re_h.sub('',item['author'])
			item['dynasty'] = re_h.sub('',item['dynasty'])
